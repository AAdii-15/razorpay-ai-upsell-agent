...
def test_pop_pending_decision_is_atomic_under_real_concurrency(isolated_audit_db):
    """20 real threads racing to pop the SAME decision — exactly one must
    win. This is a genuine double-charge risk if it regresses: /approve
    calls pop_pending_decision and then fires a real Razorpay payment."""
    import audit
    import threading

    audit.save_pending_decision("race_dec", "sess", {"final_price_paise": 100})

    results = []
    results_lock = threading.Lock()

    def worker():
        r = audit.pop_pending_decision("race_dec")
        with results_lock:
            results.append(r)
    ...
    assert len(successes) == 1, f"{len(successes)} of 20 concurrent threads got the pending decision — must be exactly 1"
